...
Chain* Builder::createLoad(FrameIndex* ptr, Value* ret) {
    auto load = std::make_unique<Chain>(Node::NodeKind::Load, ret);
    auto retPtr = load.get();
    load->addOperand(ptr);
    insert(std::move(load));
    return retPtr;
}

Chain* Builder::createStore(FrameIndex* ptr, Node* value) {
    auto store = std::make_unique<Chain>(Node::NodeKind::Store);
    auto retPtr = store.get();
    store->addOperand(ptr);
    store->addOperand(value);
    insert(std::move(store));
    return retPtr;
}

Chain* Builder::createReturn(Node* value) {
    auto ret = std::make_unique<Chain>(Node::NodeKind::Ret);
    auto retPtr = ret.get();
    if(value)
...
Chain* Builder::createCondJump(Root* first, Root* second, Node* cond) {
    auto jump = std::make_unique<Chain>(Node::NodeKind::Jump);
    auto retPtr = jump.get();
    jump->addOperand(first);
    jump->addOperand(second);
    jump->addOperand(cond);
    insert(std::move(jump));
    return retPtr;
}

Chain* Builder::createJump(Root* to) {
    auto jump = std::make_unique<Chain>(Node::NodeKind::Jump);
    auto retPtr = jump.get();
    jump->addOperand(to);
    insert(std::move(jump));
...
Chain* Builder::createCall(Value* result, Node* callee, const std::vector<Node*>& args, bool isResultUsed) {
    auto call = std::make_unique<Call>(result, callee, isResultUsed);
    auto retPtr = call.get();
    for(auto arg : args)
        call->addOperand(arg);
...
Chain* Builder::createSwitch(Node* value, Root* defaultCase, const std::vector<std::pair<Node*, Root*>>& cases) {
    auto switchInst = std::make_unique<Chain>(Node::NodeKind::Switch);
    auto retPtr = switchInst.get();
    switchInst->addOperand(value);
    switchInst->addOperand(defaultCase);
    for(auto casePair : cases) {
        switchInst->addOperand(casePair.first);
        switchInst->addOperand(casePair.second);
    }
    insert(std::move(switchInst));
    return retPtr;
}
